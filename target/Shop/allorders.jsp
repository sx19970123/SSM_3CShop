...
<head>
	...
	<header class="m-header" style="display: block;">
		...
		<div class="user">
			<div class="uc item login" id="username">${user.user_name}
			</div>
			<div class="cart item">
		...
		<section class="m-uc w">
			...
			<div class="main">
				<div class="m-uc-hd">
					<div class="title"></div>
					<div class="list">
						<a href="allOrders?user_id=${user.user_id}" class="n-active">全部订单</a>
						<a href="noPayById?user_id=${user.user_id}" class="">待支付</a>
						<a href="noDelivery?user_id=${user.user_id}" >待发货</a>
						<a href="Delivery?user_id=${user.user_id}" >待收货</a>
				...
				<div class="m-order-list">
						...
						<c:forEach items="${page.list}" var="all">
							<li class="item">
								...
								<script>
									...
									if ($.trim("${all.state}")=='2'){
										$("#l${all.cart_id}").append("<br>\n" +
												"\t\t\t\t\t\t\t\t\t\t<br>\n" +
												"\t\t\t\t\t\t\t\t\t\t<div class=\"m-header\">\n" +
												"\t\t\t\t\t\t\t\t\t\t\t<div class=\"user\">\n" +
												"\t\t\t\t\t\t\t\t\t\t\t\t<a class=\"u-btn n-middle confirm\" href=\"ConfirmReceipt?cart_id=${all.cart_id}\">确认收货</a>\n" +
												"\t\t\t\t\t\t\t\t\t\t\t</div>\n" +
												"\t\t\t\t\t\t\t\t\t\t</div>");
									}
									if ($.trim("${all.state}")=='3'){
										$("#l${all.cart_id}").append("<br>\n" +
												...
												"\t\t\t\t\t\t\t\t\t\t<div class=\"m-header\">\n" +
												"\t\t\t\t\t\t\t\t\t\t<div class=\"user\">\n" +
												"\t\t\t\t\t\t\t\t\t\t\t<a class=\"login u-btn n-middle confirm\" onclick=\"go(commodity_id=${all.commodity_id},cart_id=${all.cart_id})\">评价</a>\n" +
												"\t\t\t\t\t\t\t\t\t\t</div>\n" +
												"\t\t\t\t\t\t\t\t\t\t</div>");
									}
									if ($.trim("${all.state2}")=='6'){
										$("#l${all.cart_id}").append("<br>\n" +
												"\t\t\t\t\t\t\t\t\t\t<br>\n" +
												"\t\t\t\t\t\t\t\t\t\t<div class=\"m-header\">\n" +
												"\t\t\t\t\t\t\t\t\t\t\t<div class=\"user\">\n" +
												"\t\t\t\t\t\t\t\t\t\t\t\t<a class=\"u-btn n-middle confirm\" href=\"noPayByCartId?user_id=${user.user_id}&&cart_id=${all.cart_id}\">去支付</a>\n" +
												"\t\t\t\t\t\t\t\t\t\t\t</div>\n" +
												"\t\t\t\t\t\t\t\t\t\t</div>");
									}
								</script>
					...
					<div class="m-page">
						<c:if test="${page.hasPreviousPage}">
							<a class="front" href="allOrders?user_id=${user.user_id}&&pn=${page.prePage}"><i class="i-icon"></i></a>
						</c:if>
						<a class="page n">${page.pageNum}</a><span>/</span>
						<a class="page">${page.pages}</a>
						<c:if test="${page.hasNextPage}">
							<a class="after" href="allOrders?user_id=${user.user_id}&&pn=${page.nextPage}"><i class="i-icon"></i></a>
						</c:if>
					</div>
				</div>
			</div>
	...
	<%--评价框--%>
	<form action="addComment" method="post" id="comment">
		<section class="m-login-modal" style="display:none;">
			<div class="m-modal">
				<input name="commodity_id" id="commodity_id" value="">
				<input name="user_id" value="${user.user_id}">
				<input name="cart_id" id="cart_id" value="">
			</div>
			<i class="bg"></i>
			<div class="wrap" style="height: auto;padding-bottom:0;">
				...
				<div class="m-form clearfix c">
					...
					<label class="bui-radios-label bui-radios-anim" style="align-content: center">
						<input type="radio" name="comment_type" value="好评" /><i class="bui-radios"></i>好评
					</label>
					&nbsp;&nbsp;
					<label class="bui-radios-label bui-radios-anim" style="align-content: center">
						<input type="radio" name="comment_type" value="中评" /><i class="bui-radios"></i>中评
					</label>
					&nbsp;&nbsp;
					<label class="bui-radios-label bui-radios-anim" style="align-content: center">
						<input type="radio" name="comment_type" value="差评" /><i class="bui-radios"></i>差评
					</label>
					<br/><br/>
					<textarea rows="10" cols="43" name="comment" id="commentsBody"></textarea>
					<div class="formitm button">
						<div class="ipt">
							<button type="button" class="u-btn n-middle confirm" onclick="comments()">评论</button>
						</div>
					</div>
				</div>
	...
	<footer class="m-footer" style="margin-top: 100px; display: block;">
		...
		<script>
			...
			//评价
			function comments() {
				if (comment.comment_type.value==''){
					alert("亲。这次购物满意不满意啊")
				} else if ($("#commentsBody").val()==''){
					alert("亲，评论里写点内容吧")
				}else {
					alert("success")
					comment.submit();
			...
			//表单传值
			function go(commodity_id,cart_id) {
